defmodule ItWhistWeb.Layouts do
  @moduledoc """
  ...
  def app(assigns) do
    ~H"""
    <header class="navbar px-4 sm:px-6 lg:px-8 bg-secondary">
      <div class="flex-1">
      ...
      <div class="flex-none ">
        <ul class="menu menu-horizontal text-primary w-full relative z-10 flex items-center gap-4 px-4 sm:px-6 lg:px-8 justify-end">
          <%= if @current_scope do %>
            ...
            <li>
              <.link href={~p"/accounts/log-out"} method="delete">Log out</.link>
            </li>
          <% else %>
            <li>
              <.link href={~p"/accounts/log-in"}>Log in</.link>
            </li>
          <% end %>
        </ul>
        <%= if @current_scope do %>
