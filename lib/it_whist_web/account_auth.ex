defmodule ItWhistWeb.AccountAuth do
  use ItWhistWeb, :verified_routes
  ...
  @remember_me_cookie "_it_whist_web_account_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_account(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ItWhistWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      ItWhistWeb.Endpoint.broadcast(account_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule ItWhistWeb.PageLive do
        use ItWhistWeb, :live_view

        on_mount {ItWhistWeb.AccountAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ItWhistWeb.AccountAuth, :require_authenticated}] do
  ...
  def on_mount(:require_admin, _params, session, socket) do
    ...
    if socket.assigns.current_scope &&
         ...
         socket.assigns.current_scope.account.is_admin do
      {:cont, socket}
    else
      socket
