defmodule ItWhistWeb.AccountAuthTest do
  use ItWhistWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias ItWhist.Accounts
  alias ItWhist.Accounts.Scope
  alias ItWhistWeb.AccountAuth

  import ItWhist.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ItWhistWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_account/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ItWhistWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ItWhistWeb.Endpoint.subscribe("accounts_sessions:#{Base.url_encode64(token)}")
      end
